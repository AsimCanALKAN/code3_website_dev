	...
	var dlabChartlist = function () {
		...
		var flotArea2 = function () {
			...
			function getRandomData2() {
				if (data_2.length > 0)
					data_2 = data_2.slice(1);
				while (data_2.length < totalPoints) {
					var prev = data_2.length > 0 ? data_2[data_2.length - 1] : 50,
						y = prev + Math.random() * 10 - 5;
					if (y < 0) {
						y = 0;
					} else if (y > 100) {
			...
			function update_plot_() {
				plot_.setData([getRandomData(), getRandomData2()]);
				plot_.draw();
				setTimeout(update_plot_, updateInterval);
			}
		}
